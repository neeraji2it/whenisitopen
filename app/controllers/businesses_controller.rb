class BusinessesController < ApplicationController
  ...
  def search
    ...
    if @ab_business_databases.empty?
      ...
      @categories = Business.select('distinct category, address, city,state,company_name,phone,id').where("category = ? and #{a} > '#{Time.now.strftime("%H").to_i - 12}' and category IS NOT NULL and address IS NOT NULL and city IS NOT NULL and address != ? and id NOT IN (?)", @ab_business_databases.first.category,@ab_business_databases.first.address,@ab_business_databases.first.id).paginate :page => params[:category_page], :per_page => 9
