  ...
  def search
    if params[:company_name].present?
      ...
      if @ab_business_databases.empty?
        ...
        @all_categories = Business.where("category = ? and city = ? and #{a} > #{Time.zone.now.strftime("%H").to_i - 12} and address IS NOT NULL and city IS NOT NULL and address != ? and id NOT IN (?)", "#{@ab_business_databases.first.category}", "#{@ab_business_databases.first.city}","#{@ab_business_databases.first.address}","#{@ab_business_databases.first.id}")
