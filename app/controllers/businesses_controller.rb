class BusinessesController < ApplicationController
  ...
  def search
    if params[:company_name].present?
      ...
      if !(params[:location_page] or params[:category_page])
        ...
        @locations = Business.near([@lat, @lng],20000, :order =>:distance).where("company_name = ? and id NOT IN (?)", "#{@business.company_name}", "#{@business.id}").paginate :page => params[:location_page], :per_page => 3
      end
    else
