class BusinessesController < ApplicationController
  ...
  def search
    ...
    params[:address].present? ? (@ab_business_databases = Business.search "(^#{params[:company_name]}, #{params[:city]}, #{params[:address]})", :limit => 1) : (@ab_business_databases = Business.search "(^#{@name}$ | (#{@name}), ^#{session[:city]})", :limit => 1)
