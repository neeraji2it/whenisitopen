  ...
  def search
    if params[:company_name].present?
      ...
      if params[:address].present?
        @ab_business_databases = Business.where("company_name = '#{@name}' and city = '#{params[:city]}' and address= '#{params[:address]}'").limit(1) 
      else
        @ab_business_databases = Business.where("company_name = '#{@name}' and city = '#{session[:city]}'").limit(1)
      end
